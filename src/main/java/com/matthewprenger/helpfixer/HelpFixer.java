...
public class HelpFixer {
    ...
    public void serverStarting(FMLServerStartingEvent event) {
        event.registerServerCommand(new CommandHelp() {
            ...
            protected List<ICommand> getSortedPossibleCommands(ICommandSender sender) {
                List<ICommand> list =
                        MinecraftServer.getServer().getCommandManager().getPossibleCommands(sender);
    ...
    public void serverStarted(final FMLServerStartedEvent event) {
        Collection<ICommand> commands =
                MinecraftServer.getServer().getCommandManager().getCommands().values();

        for (final ICommand command : commands) {
            if (!(validCompareTo(command))) {
                FMLLog.warning(String.format(
                        "[HelpFixer] Command %s incorrectly overrides compareTo: %s",
                        command.getCommandName(), command.getClass().getName()));
            }
        }
    }
